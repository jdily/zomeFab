...
(i) with different objectives, e.g.\chinky{,} maintain balance~\cite{Prevost:MIS:2013,SpinIt:Baecher:2014}, reduce size~\cite{Luo:2012:CPM}, strengthen \chireplace{structure}{structural} soundness~\cite{Zhou:2013:WSA} and generate specific sounds~\cite{Umetani:2016:PIR}, and 
...
Meanwhile, \chireplace{different internal structures are developed}{several research papers develop different internal structures}, e.g.\chinky{,} the skin-frame structure~\cite{Wang:2013:CPO}, the honeycomb-like structure~\cite{Lu:2014:BSW}, and 2D laser cutting shape proxy~\cite{Song-2016-CofiFab}.
To enable the large shape to be printed using 3D printers, Luo~\etal~\cite{Luo:2012:CPM} developed an iterative planar-cut method, aiming to fit decomposed parts in the 3D printing volume while considering factors such as \chireplace{assembility}{assemblability} and aesthetics.
Yao~\etal~\cite{Yao:2015:LPP} proposed a level-set framework for 3D shape partition and packing.
Compared to these works, our method fabricate\chinky{s} a shape with both Zometool structure and 3D-printed parts, so that we can reduce the fabrication time and cost, with the reusability of Zometool structures. 
...
\chireplace{It's}{It is} history dated back to to the 1960s where it started out as a simple construction system inspired by Buckminster Fulleresque geodesic domes, and it evolved from simple toy to versatile modeling tools through years.
Although we can use it to construct \chinky{the} complex structure, it's not intuitive for naive users to use and meanwhile time-consuming.

Tools are developed to help users to design the Zometool structures, e.g.\chinky{,} vZome \cite{SVZ} and ZomeCAD \cite{ESZ}.
These systems provide different ways to grow the structure.
\chireplace{However, the difficulties remain when it comes to build a complex structure because it lacks the ability to provide useful suggestions about what kinds of structure to use next in order to build the target shape.}{However, they cannot provide useful suggestions about what kinds of structure to use next to build the target shape. The difficulties remain when building a complex structure.}
%

\chireplace{This}{Based on the above reason, it} motivates works toward automatic construction through computational method.
...
\chireplace{The final Zometool structure is obtained by a stochastic optimization framework.}{They use a stochastic optimization framework to obtain the final Zometool structure.}
...
\chireplace{The final segmented parts can be easily assembled with optimized Zometool structure.}{The segmented parts are assembled with the optimized Zometool structure to form the final result.}
